...
public class testRLPacman extends Application {
	...
	private static void setRLAgent() {
		...
		/*
		 * pacmanmdp = new EnvironnementPacmanMDPClassic(mazename,true); EtatPacmanMDPClassic etatmdp = (EtatPacmanMDPClassic) pacmanmdp.getEtatCourant(); System.out.println("Dimensions de etatMDP: "+etatmdp.getDimensions()); FeatureFunction featurefunction = new
		 * FeatureFunctionIdentity(etatmdp.getDimensions(),4); rlagent = new QLApproxAgent(alpha,gamma,pacmanmdp,featurefunction);
		 */
		// QLearning avec approximation lineaire
		/*
		 * pacmanmdp = new EnvironnementPacmanFeatureRL(mazename,true);//smallGrid smallGrid2 mediumGrid FeatureFunction featurefunction2 = new FeatureFunctionPacman(); rlagent = new QLApproxAgent(alpha,gamma,pacmanmdp,featurefunction2);
		 */
	...
	private static void apprentissage() {
		...
		for (int i = 0; i < nbmean; i++) {
			...
			for (int nbepi = 0; nbepi < nbepisodegreedy; nbepi++) {
				...
				if (pacmanmdp.getGamepacman().isWin())
					nbwin++;
			}
		...
		/*
		 * for (int i=0;i<nbepisode;i++){ System.out.print(data[i]+" "); }
		 */
